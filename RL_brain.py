...
class DeepQNetwork:
    ...
    def _build_net(self, optimizer='rms_prop', seed=0):
        ...
        def build_layers(s,lstm_s,c_names, n_l1, n_lstm, w_initializer, b_initializer):
            ...
            with tf.compat.v1.variable_scope('l1'):
                w1 = tf.compat.v1.get_variable('w1',[n_lstm + self.n_features, n_l1], initializer=w_initializer,
                                     collections=c_names, use_resource=False)
                b1 = tf.compat.v1.get_variable('b1',[1,n_l1],initializer=b_initializer, collections=c_names, use_resource=False)
            ...
            with tf.compat.v1.variable_scope('l12'):
                w12 = tf.compat.v1.get_variable('w12', [n_l1, n_l1], initializer=w_initializer,
                                         collections=c_names, use_resource=False)
                b12 = tf.compat.v1.get_variable('b12', [1, n_l1], initializer=b_initializer, collections=c_names, use_resource=False)
            ...
            if self.dueling:
                ...
                with tf.compat.v1.variable_scope('Value'):
                    w2 = tf.compat.v1.get_variable('w2',[n_l1,1],initializer=w_initializer,collections=c_names, use_resource=False)
                    b2 = tf.compat.v1.get_variable('b2',[1,1],initializer=b_initializer,collections=c_names, use_resource=False)
                    self.V = tf.matmul(l12,w2) + b2
                # n_l1 -> n_actions
                with tf.compat.v1.variable_scope('Advantage'):
                    w2 = tf.compat.v1.get_variable('w2',[n_l1,self.n_actions],initializer=w_initializer,collections=c_names, use_resource=False)
                    b2 = tf.compat.v1.get_variable('b2',[1,self.n_actions],initializer=b_initializer,collections=c_names, use_resource=False)
        ...
        with tf.compat.v1.variable_scope('train'):
            if optimizer == 'rms_prop':
                ...
                self._train_op = \
                    tf.compat.v1.train.GradientDescentOptimizer(self.lr).minimize(self.loss)
    ...
    def do_store_reward(self, episode, time, reward):
        ...
        #file_name = 'store_reward.txt'
        #with open(file_name, 'a') as file_obj:
        #  file_obj.write("\nEpisode-" + '{:d}'.format(episode) + ", Time:" + '{:f}'.format(time) + ", Reward:" + '{:.3f}'.format(self.reward_store[episode][time]))
    ...
    def do_store_action(self,episode,time, action):
        ...
        file_name = 'store_action_w1=0.5,w2=0.5.txt'
        with open(file_name, 'a') as file_obj:
          file_obj.write("\nEpisode:" + '{:f}'.format(episode) +", : Action :" + '{:f}'.format(self.action_store[episode][time]) )
    ...
    def do_store_delay(self, episode, time, delay):
        ...
        #file_name = 'store_delay.txt'
        #with open(file_name, 'a') as file_obj:
        #  file_obj.write("\nEpisode-" + '{:d}'.format(episode) + ", Time:" + '{:f}'.format(time) + ", Delay:" + '{:.3f}'.format(self.delay_store[episode][time]))
    ...
    def do_store_energy(self, episode, time, energy):
        ...
        #file_name = 'store_energyhoose.txt'
        #with open(file_name, 'a') as file_obj:
        #  file_obj.write("\nEpisode-" + '{:d}'.format(episode) + ", Time:" + '{:.3f}'.format(time) + ", Energy:" + '{:.3f}'.format(self.energy_store[episode][time]))        
